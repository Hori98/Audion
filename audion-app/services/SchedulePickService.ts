...
const API_BASE_URL = API_CONFIG.BASE_URL; // Centralized base URL (no trailing /api)
...
class SchedulePickService {
  ...
  async getSchedulerStatus(): Promise<SchedulerStatus> {
    try {
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.AUDIO_V2.SCHEDULER_STATUS}`, {
        method: 'GET',
  ...
  async createSchedule(request: ScheduleCreateRequest): Promise<Schedule> {
    try {
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.AUDIO_V2.SCHEDULES}`, {
        method: 'POST',
        headers: await this.getHeaders(),
  ...
  async getSchedule(scheduleId: string): Promise<Schedule> {
    try {
      const path = buildPath(API_ENDPOINTS.AUDIO_V2.SCHEDULE, { id: scheduleId });
  ...
  async manuallyTriggerSchedule(scheduleId: string): Promise<AudioGenerationResponse> {
    try {
      const path = buildPath(API_ENDPOINTS.AUDIO_V2.SCHEDULE_GENERATE, { id: scheduleId });
      const response = await fetch(`${API_BASE_URL}${path}`, {
        method: 'POST',
        headers: await this.getHeaders(),
      });
  ...
  async getSchedulePlaylists(scheduleId: string, limit: number = 10): Promise<ScheduledPlaylist[]> {
    try {
      const path = buildPath(API_ENDPOINTS.AUDIO_V2.SCHEDULE_PLAYLISTS, { id: scheduleId });
      const response = await fetch(`${API_BASE_URL}${path}?limit=${limit}`, {
        method: 'GET',
        headers: await this.getHeaders(),
      });
