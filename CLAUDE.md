...
### 🚨 【CRITICAL】技術的判断における情報ソース優先度プロトコル

**絶対ルール**: 技術的判断は必ずコードベース確認を最優先とする

#### 情報ソース優先順位
1. **🥇 一次情報（最優先）**: 実際のコードベース・設定ファイル・ログ
   ```bash
   # 必須実行: 技術判断前の実装確認
   grep -r "関連キーワード" . --include="*.py" --include="*.ts"
   ls -la 該当ディレクトリ/
   ```

2. **🥈 プロジェクト情報**: README.md、CLAUDE.md、コメント・ドキュメント

3. **🥉 外部AI情報（参考程度）**: Gemini MCP等の外部相談結果
   - **注意**: 外部AIは推測・一般論で回答する可能性大
   - **必須**: コードベース確認なしに外部AI情報を確定情報として扱うことを禁止
...
## 🚀 強化された品質保証システム（v3.0 ULTRA ENHANCED）

**重要**: すべての開発作業は、三位一体の開発原則に基づく強化品質保証システムに従って実施する。
...
# AIガードレール・影響範囲分析を含む包括的検証を実行
```

**詳細**: `AI_DEVELOPMENT_QUALITY_CHECKLIST.md v3.0 (ULTRA ENHANCED)` を参照

### 新機能（v3.0強化内容）
1. **AIガードレール検証**: 修正禁止範囲・API破壊的変更の自動検知
2. **影響範囲分析**: 変更されたシンボルの参照箇所を自動特定
3. **三位一体フロー**: 人間の計画 → AIの実行 → 検証の体系化
4. **緊急事故対応プロトコル**: git revert による即座復旧体制
