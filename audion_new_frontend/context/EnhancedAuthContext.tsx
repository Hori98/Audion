...
export function EnhancedAuthProvider({ children }: EnhancedAuthProviderProps) {
  ...
  const validateToken = async (token: string): Promise<boolean> => {
    try {
      // Simple token validation - check if it can be used for API calls
      const response = await fetch(`${process.env.EXPO_PUBLIC_API_BASE_URL || 'http://192.168.11.34:8003/api'}/auth/validate`, {
