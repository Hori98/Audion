...
apiClient.interceptors.request.use(
  async (config) => {
    ...
    // Quiet default: avoid noisy per-request logs
    return config;
  },
  (error) => {
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (__DEV__) {
      // Only log subscription endpoint errors as warnings since they have fallbacks
      const isSubscriptionEndpoint = error.config?.url?.includes('/subscription/limits');
      if (isSubscriptionEndpoint) {
        console.warn('⚠️ Subscription API Error (using fallback):', {
        ...
        console.error('❌ API Error Details:', {
          ...
          requestFailed: !error.response && !!error.request,
