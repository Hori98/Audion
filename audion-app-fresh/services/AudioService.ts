...
import { API_CONFIG, API_ENDPOINTS, buildApiUrl, buildPath } from '../config/api';
...
class AudioService {
  ...
  async deleteAudioContent(
    ...
    // Build URL via shared endpoint + path params to avoid typos
    const endpoint = buildPath(API_ENDPOINTS.AUDIO.DELETE, { id: audioId });
    try {
      // Prefer axios client to reuse auth interceptor so Authorization is guaranteed
      const { data } = await apiClient.delete(endpoint);
      // Some servers may return empty body; normalize
      return data || { message: 'Audio deleted' };
    } catch (error: any) {
      const status = error?.statusCode || error?.response?.status;
      const message = error?.message || error?.response?.data?.detail || 'Failed to delete audio';
