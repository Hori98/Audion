...
export default function PromptSettingsModal({ 
  ...
  const loadSettings = async () => {
    try {
      // AsyncStorageから保存済み設定を読み込み
      const storedSettings = await AsyncStorage.getItem('prompt_settings');
      
      if (storedSettings) {
        const parsedSettings = JSON.parse(storedSettings);
        console.log('✅ Loaded prompt settings from AsyncStorage:', parsedSettings);
        setSettings(parsedSettings);
      } else {
        // 初回時またはデータがない場合はSettingsContextから読み込み
        ...
        const loadedSettings: PromptSettings = {
          auto: { 
            ...
            customPrompt: pickModes.auto.overridePromptTemplate || '' 
          },
          manual: { 
            style: currentStyle as any, 
            customPrompt: pickModes.manual.overridePromptTemplate || '' 
          },
          schedule: { 
            style: currentStyle as any, 
            customPrompt: '' 
        ...
        // 初回読み込み時にAsyncStorageにも保存
        await AsyncStorage.setItem('prompt_settings', JSON.stringify(loadedSettings));
        console.log('✅ Initial prompt settings saved to AsyncStorage');
      }
    } catch (error) {
  ...
  const performSave = async (newSettings: PromptSettings, mode: keyof PromptSettings, field: string, value: any) => {
    try {
      // 1. AsyncStorageに即座保存（最重要 - タブ切り替え時の消失防止）
      await AsyncStorage.setItem('prompt_settings', JSON.stringify(newSettings));
      console.log(`💾 Saved to AsyncStorage: ${mode}.${field} =`, value);
      
      // 2. SettingsContextを通じて保存（アプリ内状態同期）
      if (field === 'style') {
        await updateContent({ promptTemplate: value });
      }
      if (field === 'customPrompt') {
        // カスタム文面は各モードのoverrideに保存（AutoPick優先で反映される）
        if (mode === 'auto') {
          await updatePickModes({ auto: { overridePromptTemplate: value } });
        } else if (mode === 'manual') {
          await updatePickModes({ manual: { overridePromptTemplate: value } });
        } else if (mode === 'schedule') {
          await updatePickModes({ schedule: { /* 将来の拡張 */ } as any });
        }
        // スタイルがcustomでない場合でも、文面は保存だけしておく
      }
      ...
      Alert.alert(
        '保存エラー', 
        `設定の保存に失敗しました: ${error.message || error}`,
        [{ text: 'OK' }]
      );
    }
  };
