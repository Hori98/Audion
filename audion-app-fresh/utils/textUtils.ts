...
export const getCleanScriptText = (xmlString: string): string => {
  ...
  // React Native環境ではDOMParserが存在しないため、正規表現フォールバックを優先
  // これによりReferenceErrorによるRedBoxを回避
  // （バックエンドと同じ抽出結果になるようstrip/整形も実施）
  // 環境によってはglobal.DOMParserが存在しても制限されている場合があるため防御的に判定
  ...
  try {
    ...
    // React Nativeではconsole.errorがRedBoxを出すためwarnに変更
    console.warn('XML parse error, falling back to regex:', error);
    return extractScriptWithRegex(xmlString);
  }
};
