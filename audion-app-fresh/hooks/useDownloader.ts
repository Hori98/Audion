...
export const useDownloader = () => {
  ...
  const MIN_VALID_AUDIO_BYTES = 2048;

  const sanitizeId = (raw: string) => raw.replace(/[^A-Za-z0-9_-]/g, '_');
  ...
  const getLocalUri = (id: string, remoteUrl?: string) => {
    ...
    return `${FileSystem.documentDirectory}audio_${safeId}.${fileExtension}`;
  };
  ...
  const downloadAudio = async (audio: AudioMetadata): Promise<boolean> => {
    ...
    try {
      ...
      if (result) {
        ...
        if (!fileInfo.exists || (fileSize ?? 0) < MIN_VALID_AUDIO_BYTES) {
          try {
            if (fileInfo.exists) {
          ...
          console.warn('Downloaded file is too small or missing, treated as invalid:', audio.id, fileSize);
          return false;
        }
